...
The repo ships with the **50-deck evaluation corpus**, **deck-only ground truth**, **frozen pipeline outputs**, **evaluation summaries**, and **inter-rater agreement data** (Fleiss' ? = 0.78 on the ten-deck pilot).
...
?       ??? kappa.csv           # Fleiss' ? computation (? ? 0.78)
?
...
| `bmc_jonas_k.csv` | Second annotator |
| `bmc_max_k.csv` | Third annotator |
| `kappa.csv` | Binary fill/empty labels + Fleiss' ? math (? ? 0.78) |

High ? confirms consistent **fill presence** under the protocol; it does not measure wording similarity (that is evaluated separately in Module 02).
